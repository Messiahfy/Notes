...
> 完整的自定义UI组件需要分别继承`RenderObjectWidget`、`RenderObjectElement`和`RenderObject`三件套，比如`Stack`就实现了`MultiChildRenderObjectWidget`、`MultiChildRenderObjectElement`和`RenderStack`，在`RenderStack`中自定义了布局和绘制流程。不过由于Flutter是声明式组合设计，所以多数情况其实不需要我们完整的去继承实现，而是利用Flutter官方提供的实现来组合完成，或者基于`CustomMultiChildLayout`、`CustomSingleChildLayout`、`CustomPaint`来快速实现。
