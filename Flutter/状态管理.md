...
```
> `InheritedWidget`会创建`InheritedElement`，而`Element`在`mount`时会调用`updateInheritance`方法，`InheritedElement`重写该方法，会把当前`InheritedWidget`实现类运行时类型作为key，把自己（即`InheritedElement`）作为value，保存到`_inheritedWidgets`中，并且`_inheritedWidgets`实例为所有Element共享。**所以在这里可以通过`_inheritedElements`查找到指定`InheritedWidget`对应的`InheritedElement`**

这里将会找到祖先元素中的`InheritedElement`，调用`dependOnInheritedElement`：
```
@override
...
```
`ProxyElement`的`update`方法调用`InheritedElement`的`notifyClients`方法，就会遍历所有依赖InhheritedWidget的Widget对应的Element执行rebuild，从而导致widget重建。

对于`StatefulWidget`对应的`StatefulElement`类型，重写了`didChangeDependencies`方法，子元素就会把`_didChangeDependencies`改为true。并且`StatefulElement`重写了`performRebuild()`，会调用`State`的`didChangeDependencies`方法。：
```
@override
void performRebuild() {
...
## InheritedNotifier
InheitedWidget并不是StatefulWidget，而要让child widget更新，又需要触发InheitedWidget rebuild，所以会把InheitedWidget实现类的对象放到StatefulWidget中，触发StatefulWidget的rebuild，从而触发InheitedWidget rebuild。但其实我们想rebuild的只是InheitedWidget和依赖它的widget并不想影响StatefulWidget内的其他不需要更新的widget。

`InheritedNotifier`是`InheitedWidget`的子类，它会持有一个`Listenable`对象，一般使用`ChangeNotifier`等子类，通过`Listenable`对象建立数据共享和通知机制，调用`Listenable`的`notifyListeners()`时，会导致`InheritedNotifier`的Element也就是`_InheritedNotifierElement`执行`markNeedsBuild()`，但触发`_InheritedNotifierElement`的build也只是直接返回`InheritedNotifier`的child对象，并不会像`StatefulWidget`那样重新执行`State`的`build`方法。所以会比直接使用`InheritedWidget`性能更好。（其实就相当于自定义了`InheritedWidget`的Element，并且通过持有的`Listenable`对象通知变化触发Element的build，但会直接返回child，不会像`StatefulWidget`的那样重新执行State.build函数）
