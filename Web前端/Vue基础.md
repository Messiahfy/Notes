...
# 开发模式
现在 Vue（以 Vue 3 为主）的主流开发模式基本可以分成两块：工程化/基础设施（项目怎么搭、怎么跑、怎么发布）和组件/业务开发范式（代码怎么写、状态怎么管、怎么通信）。

## 一、工程化标配（常见组合）

- 构建工具 ：Vite
- 路由 ：vue-router（SPA/多页面都常用）
- 状态管理 ：Pinia（Vue 3 官方推荐，取代 Vuex）
- HTTP ：axios 封装
- UI 组件库 ：Element Plus / Ant Design Vue
- 样式方案 （二选一或混用）：
  - 原生 CSS + scoped
  - CSS 预处理：Sass/Less
  - 原子化：Tailwind CSS（越来越常见）
- 代码质量 （“企业标配”很常见）：
  - ESLint（规范 JS/TS 与 Vue SFC）
  - Prettier（格式化）
  - stylelint（可选，管理 CSS）
  - commitlint + lint-staged + husky（可选，管提交质量）
- 类型系统 ：TypeScript（中大型项目基本标配）
- Mock ：mockjs / MSW（前端更偏 MSW，mockjs 老牌但仍常用）
- 测试 （按团队成熟度选）：
  - 单测：Vitest / Jest
  - 组件测试：@vue/test-utils
...
- 按需构建/优化 ：
  - 路由懒加载（import 动态导入）
  - 图标按需（如 unplugin-icons），组件按需（element-plus 的按需引入/自动导入）

## 二、Vue 组件开发的主流写法

主流是用 `Composition API`（ ref/reactive/computed/watch ）+ `<script setup>` 语法糖：

- 响应式基础 ：
  - ref() ：基本类型/单值、DOM 引用、以及很多业务状态都用它
  - reactive() ：更偏对象集合（但在组件里 ref + 对象也很常见）
  - computed() ：派生状态
  - watch()/watchEffect() ：副作用、联动、请求触发
- 组件通信 ：
  - defineProps() ：接收父组件参数
  - defineEmits() ：向父组件发事件
  - defineModel() ：Vue 3.4+ 常用来做 v-model 双向绑定的语法糖（等价于 props+emit 的特定组合），做表单组件/弹窗组件很方便
  - provide/inject ：跨层级依赖注入（主题、表单上下文等）
- 组合式复用 ：
  - 把逻辑提到 composable（ useXxx() ）里：例如 useTable 、 useFetch 、 useUser ，这是 Vue 3 非常主流的复用方式
- 请求与数据流 ：
  - 常见是“页面组件”只负责调用 api.js 的函数并赋值到 ref ，请求细节（token、错误、解包）放到 request.js 这类基础层

## 三、简单总结：

- 组件内部状态： ref/reactive 管
- 父子输入输出： defineProps/defineEmits （或 defineModel ）管
- 状态跨页面/跨组件共享：Pinia 管
- 副作用（请求、监听、定时器）： onMounted/watch 管
...
`v-model` = `v-bind:value（或者简写为 :value）`（数据从 Vue 流向 DOM / 组件） + `v-on:click（或者简写为 @click）`（数据从 DOM / 组件流向 Vue），双向联动。
...
### 父子通信简单总结
* 向下传值：v-bind（或者简写:）发出，props 接收
* 向上通信：emit 发出，v-on（或者简写@）接收

### defineModel()
`defineModel()` 用于实现父子组件双向绑定 v-model，省去：props + emit + 封装computed 样板代码。

父级：
```html
<template>
  <!-- 直接绑定 defineModel 返回的变量 -->
  <input v-model="value" />
</template>

<script setup>
// 自动接收 modelValue、自动触发 update:modelValue
...
watch(userId, async (newId, oldId) => {
  console.log(`用户从 ${oldId} 变成了 ${newId}`); // 可以拿到旧值
...
// 不告诉 Vue 监听谁，只描述要做什么
watchEffect(async () => {
  // Vue 会自动发现读取了 userId.value，就把它加进依赖列表
  // 下次 userId 变了，这个函数就会重新执行
  const data = await fetchUser(userId.value); 
  userInfo.value = data;
  // ⚠️ 缺点：拿不到 oldId；而且每次重跑都会重新发起请求
});
```
