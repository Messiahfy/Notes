...
&emsp;&emsp;`findNextFocus`方法会先调用`findNextUserSpecifiedFocus()`查找指定的下个焦点（例如`nextFocusLeft`），如果没有指定，则会调用`addFocusables()`将自身（如果是focusable）和可见的child View（如果是focusable）添加到待确定焦点的focusable视图集合中，children在添加到集合前会先进行排序，从上到下、从左到右(`layoutDirection`为`rtl`则从右到左)。parent和children的优先顺序可以通过`setDescendantFocusability()`来确定parent先于、后于还是限制children获取焦点。<br/>
